...
def decode_jwt(token: str) -> dict:
    try:
        # Check if tokein is blacklisted
        # user_repository = UserRepository()
        # is_blacklisted = user_repository.check_if_token_blacklisted(token)
    ...
    def verify_jwt(self, jwt_token: str) -> bool:
        ...
        if payload:
            ...
            with Database(configs.DATABASE_URI).session() as db:
                blacklisted = db.query(TokenBlacklist).filter(TokenBlacklist.access_token == jwt_token).first()
